...
const ProfileField: React.FC<ProfileFieldProps> = ({ label, value, editing, onChange, isDate = false }) => {
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-md transition-all duration-300 hover:shadow-lg">
      <span className="text-xs font-semibold uppercase text-indigo-500 tracking-wider">{label.replace(/([A-Z])/g, " $1").trim()}</span>
      {editing ? (
        <input
        ...
        <span className="text-gray-800 font-medium text-lg block mt-1 wrap-break-word">{displayValue}</span>
      )}
    </div>
  );
};
...
interface UserProfileProps { userType?: "admin" | "hr"; }
...
const UserProfile: React.FC<UserProfileProps> = ({ userType = "admin" }) => {
  ...
  const allMappedKeys = useMemo(() => {
    ...
    Object.values(BASE_SECTION_MAP).forEach(sectionKeys => sectionKeys.forEach(k => keys.add(k)));
    return keys;
  }, []);

  const getImageUrl = (img?: string) => img ? (img.startsWith("http") ? img : `https://res.cloudinary.com/dk9i3x5la/image/upload/${img.replace(/^uploads\//, "")}`) : null;
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        if (!userData || !userData._id) { setErrorMsg(res.data.message || "User not found."); setLoading(false); return; }

        setUser(userData); setFormData(userData);

        const allDataKeys = Object.keys(userData) as (keyof EmployeeProfileData)[];
        const uncategorizedKeys = allDataKeys.filter(key => !allMappedKeys.has(key) && !EXCLUDED_KEYS.includes(key as string));
        const newMap: Record<string, (keyof EmployeeProfileData)[]> = { ...BASE_SECTION_MAP };
        if (uncategorizedKeys.length) newMap["Other / Uncategorized"] = uncategorizedKeys.sort();
        setEffectiveSectionMap(newMap);

      } catch (err: unknown) {
        if (axios.isAxiosError(err)) setErrorMsg(err.response?.data?.message || err.message);
  ...
  // -------------------- Handlers --------------------
  const handleInputChange = (key: keyof EmployeeProfileData, value: string) => setFormData(prev => ({ ...prev, [key]: value }));
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (!EXCLUDED_KEYS.includes(key)) {
          dataToSubmit[key as keyof EmployeeProfileData] = formData[key as keyof EmployeeProfileData];
        }
      });

      console.log("PUT payload to backend:", dataToSubmit);
      if (!Object.keys(dataToSubmit).length) { toast("No changes detected."); setEditing(false); return; }

      const res = await axios.put(updateUrl, dataToSubmit, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleCancel = () => { setEditing(false); setFormData(user || {}); toast.success("Editing cancelled."); };
  const currentGradientStyle = { background: `linear-gradient(90deg, ${gradients[gradientIndex][0]}, ${gradients[gradientIndex][1]})`, transition: "background 1s ease-in-out" };
  ...
  if (!user) return <p className="text-center mt-10 text-gray-500">No profile data available.</p>;

  const filteredFields = Object.entries(formData)
    .filter(([key]) => effectiveSectionMap[activeSection as string]?.includes(key as keyof EmployeeProfileData))
        ...
        <div className="max-w-4xl mx-auto bg-white shadow-3xl rounded-3xl overflow-hidden relative border border-gray-200">
          ...
          <div style={currentGradientStyle} className="p-8 sm:p-10 flex flex-col sm:flex-row items-center sm:items-start gap-6 relative">
            <div className="absolute top-4 right-4 flex space-x-2">
              {editing && <button className="p-3 rounded-full bg-white text-gray-700 shadow-md hover:bg-gray-200 transition-all border border-gray-200" onClick={handleCancel}><FiX className="w-5 h-5" /></button>}
              <button className="p-3 rounded-full bg-white text-indigo-600 shadow-lg hover:bg-gray-100 hover:shadow-xl transition-all" onClick={() => editing ? handleSave() : setEditing(true)}>
            ...
            <div className="shrink-0">
              {imageUrl ? (
                <div className="relative w-32 h-32 rounded-full border-4 border-white shadow-xl overflow-hidden">
                  <Image src={imageUrl} alt={`${user.firstName} ${user.lastName}`} fill className="object-cover" />
                </div>
              ) : (
                <div className="w-32 h-32 rounded-full border-4 border-white flex items-center justify-center text-white font-extrabold text-4xl shadow-xl bg-gray-300">{user.firstName?.[0] ?? "U"}</div>
              )}
            </div>
          ...
          <div className="border-b border-gray-200 px-6 sm:px-10 pt-4">
            <div className="-mb-px flex space-x-4 sm:space-x-8 overflow-x-auto pb-1">
              {sectionKeys.map((section) => (
                <button key={section} onClick={() => { if (editing) { toast.error("Save or cancel edits first"); return; } setActiveSection(section); }} className={`py-3 px-3 sm:px-4 text-sm font-medium transition-colors duration-200 whitespace-nowrap ${activeSection === section ? "border-b-4 border-indigo-600 text-indigo-700" : "border-b-4 border-transparent text-gray-500 hover:text-indigo-500 hover:border-gray-300"}`} disabled={editing}>
                  {section}
                </button>
              ))}
            </div>
            ...
            {filteredFields.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredFields.map(([key, value]) => (
                  <ProfileField key={key} label={key} value={(value ?? "").toString()} editing={editing} isDate={key.toLowerCase().includes("date") || key.toLowerCase().includes("joining") || key.toLowerCase().includes("leaving")} onChange={(val) => handleInputChange(key as keyof EmployeeProfileData, val)} />
                ))}
              </div>
            ) : (
              <div className="text-gray-500 p-8 border border-dashed rounded-xl bg-gray-50 text-center">No editable fields available for {activeSection}.</div>
            )}
          </div>

          {/* Footer */}
          <div className="p-8 flex justify-start bg-gray-50 border-t border-gray-100">
            <button onClick={() => router.back()} className="flex items-center gap-2 bg-indigo-600 text-white px-6 py-3 rounded-xl hover:bg-indigo-700 transition shadow-lg"><FiArrowLeft /> Back</button>
          </div>
        </div>
      </div>
    </>
