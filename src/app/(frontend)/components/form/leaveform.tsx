...
const fetchLeaves = useCallback(async () => {
  try {
    ...
    let sortedLeaves: Leave[] = Array.isArray(data.leaves)
      ? data.leaves.sort(
          (a: Leave, b: Leave) =>
    ...
    // Filter leaves based on userRole
    if (userRole === "teamlead") {
      const userEmail = localStorage.getItem("userEmail");
      sortedLeaves = sortedLeaves.filter((l) =>
        l.approvers.teamLead === userEmail || l.email === userEmail
      );
    }
...
const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
  ...
  try {
    const approversPayload: Leave["approvers"] = {
      teamLead: null,
      hr: null,
    };

    // Only set approvers for a normal user
    if (userRole === "user") {
      // TeamLead needs to approve first
      approversPayload.teamLead = "teamlead_email_or_id"; // dynamically get your team lead
      approversPayload.hr = null; // HR approves after team lead
    } else if (userRole === "teamlead") {
      // TeamLead submits on behalf of user? Usually not, skip
      approversPayload.hr = "hr_email_or_id"; // HR still approves
    }
    ...
    const res = await fetch("/api/user/profile/request/leave", {
      ...
      body: JSON.stringify({ ...formData, approvers: approversPayload }),
    });
    ...
    <div className="">
      ...
      <div className="flex justify-between items-center pb-4 border-b-4 border-indigo-200/50">
        ...
        <button
          ...
          className="flex items-center bg-green-600 text-white px-6 py-2.5 rounded-xl font-semibold shadow-lg shadow-indigo-400/50 hover:bg-indigo-700 transition-all duration-300 transform hover:scale-[1.02] focus:outline-none focus:ring-4 focus:ring-indigo-300"
        >
          + Request New Leave
        </button>
      </div>
