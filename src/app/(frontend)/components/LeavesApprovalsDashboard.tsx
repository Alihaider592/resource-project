...
//       const apiUrl =
//         userRole === "user"
//           ? "/api/user/profile/request/leave" // user sees only own leaves
//           : "/api/hr/leaves/all"; // teamlead & hr see all leaves
...
const LeaveApprovalDashboard: React.FC<Props> = ({ userRole }) => {
  ...
  // -------------------- FIXED --------------------
  const computeLeaveStatus = (leave: Leave): "pending" | "approved" | "rejected" => {
    const decisions = Object.values(leave.approverStatus || {});
    if (decisions.includes("reject")) return "rejected";

    // Only count approvers that actually exist
    const validApprovers = Object.values(leave.approvers || {}).filter(v => v);

    if (decisions.length === validApprovers.length && decisions.every(d => d === "approve"))
      return "approved";
    ...
    try {
      ...
      if (res.ok) {
        setLeaves(prev =>
          prev.map(l => {
            if (l._id === leaveId) {
              const updated: Leave = { ...l };
              updated.approverStatus = { ...updated.approverStatus, [userRole]: action };
              updated.approverComments = [
                ...(updated.approverComments || []),
                {
                      ...
                      <tr className="bg-white border-t border-indigo-200/50 shadow-inner">
                        <td colSpan={5} className="p-6">
                                ...
                                <ul className="space-y-2 text-sm">
                                  {leave.approverComments.map((c, idx) => (
                                    <li
                                      key={idx}
                                      className={`flex items-start ${
                                        c.action === "reject" ? "text-red-700" : "text-green-700"
                                      }`}
                                    >
                                      <FiClock className="w-4 h-4 mt-0.5 mr-2 flex-shrink-0 text-gray-500" />
                                      <div className="flex-grow">
                                        ...
                                        <span
                                          className={`font-medium ${
                                            c.action === "reject" ? "text-red-600" : "text-green-600"
                                          }`}
                                        >
                                          {c.action}d
                                        </span>{" "}
                          ...
                          {hasActed && (
                            ...
                            >
                              <p>
                                {approverDecision === "approve" && <FiCheckCircle className="inline w-5 h-5 mr-2" />}
                                {approverDecision === "reject" && <FiXCircle className="inline w-5 h-5 mr-2" />}
                                You have already <strong>{approverDecision}d</strong> this request.
                              </p>
                            </div>
                          )}
                        </td>
