...
import Cmap from '@/app/(forntend)/components/frame/frameCards/cmap/cmap';
const Page = () => {
    ...
    <div className="flex flex-col items-center justify-center gap-20 p-20">
      ...
      <div className="flex flex-col gap-8 w-full overflow-hidden mt-20">
        {logoLines.map((logos, index) => {
          const { duration, direction } = lineConfigs[index % lineConfigs.length];
      ...
      <div className='gap-10 relative right-52 flex flex-col '>
        ...
        <p className='text-md'>Experience all of the features without limitations.  Attendance, Leave, Recruitment, and Payroll.</p>
        <button className='bg-blue-500 w-40 hover:bg-gray-500 transition-all duration-500 rounded-4xl text-white font-bold p-4'>Start free trial</button>
      </div>
        <img src="https://resourceinn.com/wp-content/uploads/2023/01/footer-trial-form-01.webp" alt="" />
        </div>
        </div>
  );
};
